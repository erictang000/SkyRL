...
def compute_score_centering_head_logprobs(
    ...
    *,
    ...
    Aligns the packed support rows to the model's token layout (padded or THD-packed), scores the members
    on this rank's vocabulary shard and returns ``[batch, num_actions, k]`` in canonical batch order,
    ``-inf`` on padding members. ``token_logprobs`` are the sampled-token logprobs in canonical
    ``[batch, seq_len - 1]`` (or ``[batch, seq_len]``) order as produced by the standard logprob path.
    """
    if packed:
    ...
    if renormalize_over_head:
        ...
        return member_logprobs[:, -num_actions:]
    ...
    return torch.where(valid, member_logits - log_z.unsqueeze(-1), float("-inf"))
