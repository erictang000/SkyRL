...
def prepare_runtime_environment(cfg: SkyRLTrainConfig) -> dict[str, str]:
    ...
    if cfg.trainer.strategy == "megatron":
        ...
        # Propagate fla's GDN backend choice to Ray workers only when the user set it.
        # Unset, fla picks per hardware: TileLang on Hopper with Triton >= 3.4 (where the
        # Triton GDN backward is broken, fla#640) and Triton elsewhere. Forcing "1" here
        # made Blackwell (B200) run the TileLang packed backward, which aborts.
        if "FLA_TILELANG" in os.environ:
            env_vars["FLA_TILELANG"] = os.environ["FLA_TILELANG"]
