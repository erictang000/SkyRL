...
# using megatron backend for training
# bash examples/train/algorithms/dapo/prepare_dapo_data.sh
# bash examples/train/algorithms/dapo/run_dapo_qwen3_1.7b_fully_async_megatron.sh
...
ROLLOUT_KL_LOSS_COEF=2e-3
USE_ROLLOUT_KL_LOSS=true

MAX_TOKENS_PER_MICROBATCH=16000

RUN_NAME=dapo_qwen3_1.7b_base-async-dppo-${DPPO_TYPE}-deltaLow${DPPO_DELTA_LOW}-deltaHigh${DPPO_DELTA_HIGH}-maxStale${MAX_STALENESS_STEPS}_rollout_kl_loss${ROLLOUT_KL_LOSS_COEF}
...
uv run --isolated --extra megatron -m examples.train.algorithms.dapo.main_dapo_fully_async \
  ...
  trainer.algorithm.use_rollout_kl_loss=$USE_ROLLOUT_KL_LOSS \
  trainer.algorithm.rollout_kl_loss_coef=$ROLLOUT_KL_LOSS_COEF \
  ...
  trainer.project_name="dapo-async-megatron" \
  trainer.run_name="$RUN_NAME" \
  trainer.export_path="/mnt/nvme/exports/$RUN_NAME" \
  trainer.hf_save_interval=$EVAL_CKPT_INTERVAL \
  trainer.resume_mode=latest \
  trainer.max_ckpts_to_keep=3 \
  trainer.ckpt_path="/mnt/nvme/ckpts/$RUN_NAME" \
  $@
