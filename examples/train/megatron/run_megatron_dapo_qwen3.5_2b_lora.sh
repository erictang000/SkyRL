...
# Colocated DAPO training+generation for Qwen3.5-2B-Base on DAPO training data and validate on AIME 2024.
# bash examples/train/algorithms/dapo/prepare_dapo_data.sh
# bash examples/train/megatron/run_megatron_dapo_qwen3.5_2b_lora.sh

MODEL_NAME="$HOME/models/Qwen3.5-2B-Base"
...
uv run --isolated --extra megatron -m examples.train.algorithms.dapo.main_dapo \
  ...
  trainer.run_name="dapo_qwen3.5_2b_base_lora" \
  trainer.export_path="$HOME/exports/dapo_qwen3.5_2b_base_lora" \
  trainer.hf_save_interval=-1 \
  trainer.resume_mode=latest \
  trainer.max_ckpts_to_keep=3 \
  trainer.ckpt_path="$HOME/ckpts/dapo_qwen3.5_2b_base_lora" \
  $@
