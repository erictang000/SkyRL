...
2. **Drafting with it** — the trained head is handed to vLLM as an MTP drafter. Every weight sync reloads it in vLLM's native draft session, right after the policy.
...
<Callout type="warn">
Syncing the drafter requires the `megatron` backend and the `nccl` (default) or `delta` weight sync backend. Sharded RDT, serialized FP8 weight sync, and adapter-only LoRA (`merge_lora=false`) are rejected at config validation.
</Callout>
