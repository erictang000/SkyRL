...
- `generator.inference_engine.delta_weight_sync.sync_dir`: Shared local/NFS directory, a GCS (`gs://..`) URI or an S3 (`s3://...`) URI where the trainer publishes versioned delta artifacts. Required when `weight_sync_backend=delta`.
